...
namespace Order.Persistence.Repository
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public Repository(OrderDbContext db, ILogger<Repository<T>> logger)
        {
            _db = db;
            EntitySet = _db.Set<T>();
            _logger = logger;
        ...
        public T InsertResult(T entidad)
        {
            try
            {
                _logger.LogInformation($"Repository => InsertResult<{typeof(T).Name}> => Inserta");
                EntitySet.Add(entidad);

                _logger.LogInformation($"Repository => InsertResult<{typeof(T).Name}> => Guarda cambios");
                _db.SaveChanges();
        ...
        public bool InsertRange(List<T> entidad)
        {
            try
            {
                _logger.LogInformation($"Repository => InsertRange<{typeof(T).Name}> => Inserta");
                EntitySet.AddRange(entidad);

                _logger.LogInformation($"Repository => InsertRange<{typeof(T).Name}> => Guarda cambios");
                return _db.SaveChanges() > 0;
            }
            catch (Exception)
            {
